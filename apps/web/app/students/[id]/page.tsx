...
export default async function StudentDossierPage({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:student},{data:documents},{data:enrollments},{data:years},{data:grades},{data:assessments},{data:subjects}] = await Promise.all([
 ...
 if(!student) notFound()
 return <StudentDossier organizationId={member.organization_id} student={student} documents={documents??[]} enrollments={enrollments??[]} years={years??[]} grades={grades??[]} assessments={assessments??[]} subjects={subjects??[]} />
}